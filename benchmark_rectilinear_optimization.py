...
def create_test_grids(nx_source=100, ny_source=50, nx_target=50, ny_target=25):
    ...
    source_ds = xr.Dataset(
        {"temperature": (("lat", "lon"), np.random.random((ny_source, nx_source)).astype(np.float32), {"units": "K"})},
        coords={"lat": (["lat"], lat_1d_source), "lon": (["lon"], lon_1d_source)},
    )
    ...
    target_ds = xr.Dataset(coords={"lat": (["lat"], lat_1d_target), "lon": (["lon"], lon_1d_target)})
...
def benchmark_regridding():
    ...
    assert np.allclose(np.asarray(result1.values), np.asarray(result2.values), equal_nan=True), (  # noqa: S101
