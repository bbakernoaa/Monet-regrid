...
from xarray_regrid.utils import (
    _get_grid_type,
    format_for_regrid,
...
class Regridder:
    ...
    def build_regridder(
        self, ds_target_grid: xr.Dataset, method: str = "linear", **kwargs
    ) -> BaseRegridder:
            ...
            try:
                ...
                for coord_name in self._obj.coords:
                    ...
                    if coord_var.ndim >= 1 and any(
                        keyword in str(coord_name).lower()
        ...
        if (
            source_grid_type == GridType.CURVILINEAR
            or target_grid_type == GridType.CURVILINEAR
        ):
    ...
    def linear(
        ...
        if (
            source_grid_type != GridType.CURVILINEAR
            and target_grid_type != GridType.CURVILINEAR
        ):
        ...
        regridder = self.build_regridder(
            ds_target_grid=ds_target_grid, method="linear", time_dim=time_dim
        )
        return regridder()
    ...
    def nearest(
        ...
        regridder = self.build_regridder(
            ds_target_grid=ds_target_grid, method="nearest", time_dim=time_dim
        )
        return regridder()
    ...
    def cubic(
        ...
        regridder = self.build_regridder(
            ds_target_grid=ds_target_grid, method="cubic", time_dim=time_dim
        )
        return regridder()
...
def validate_input(
    ...
    def _find_coordinate_matches(source_coords, target_coords):
        ...
        source_lat_coords = [
            c for c in source_coords if any(p in str(c).lower() for p in lat_patterns)
        ]
        source_lon_coords = [
            c for c in source_coords if any(p in str(c).lower() for p in lon_patterns)
        ]
        target_lat_coords = [
            c for c in target_coords if any(p in str(c).lower() for p in lat_patterns)
        ]
        target_lon_coords = [
            c for c in target_coords if any(p in str(c).lower() for p in lon_patterns)
        ]

        # If we have both lat and lon coordinates in both source and target, we have matches
        if (
            source_lat_coords
            and source_lon_coords
            and target_lat_coords
            and target_lon_coords
        ):
    ...
    coord_matches = _find_coordinate_matches(
        list(data.coords), list(ds_target_grid.coords)
    )
    ...
    if len(coord_matches) == 0:
        ...
        if len(dim_matches) == 0:
            ...
            semantic_dim_matches = _find_coordinate_matches(
                list(data.dims), list(ds_target_grid.dims)
            )
