...
class BaseRegridder(abc.ABC):
    ...
    def __init__(
        self, source_data: xr.DataArray | xr.Dataset, target_grid: xr.Dataset, **kwargs
    ):
    ...
    def _identify_lat_coords(self, data):
        ...
        if hasattr(data, "dims"):
            for dim in data.dims:
                dim_lower = str(dim).lower()
                if any(
                    keyword in dim_lower for keyword in ["lat", "latitude", "yc", "y"]
                ):
                    return [dim]
    ...
    def _identify_lon_coords(self, data):
        ...
        for name in data.coords:
            name_lower = str(name).lower()
            if any(
                keyword in name_lower for keyword in ["lon", "longitude", "xc", "x"]
            ):
                return [name]
        ...
        if hasattr(data, "dims"):
            for dim in data.dims:
                dim_lower = str(dim).lower()
                if any(
                    keyword in dim_lower for keyword in ["lon", "longitude", "xc", "x"]
                ):
                    return [dim]
...
class RectilinearRegridder(BaseRegridder):
    ...
    def __call__(
        self, data: xr.DataArray | xr.Dataset | None = None, **kwargs
        ...
        method_kwargs = {
            **self.method_kwargs,
            **{k: v for k, v in kwargs.items() if k not in ["method", "time_dim"]},
        }

        # Import here to avoid circular imports
        from xarray_regrid.methods import conservative, interp
        ...
        if cache_key in self._validation_cache:
            ...
            # Validate inputs
            validated_target_grid = validate_input(
                input_data, self.target_grid, time_dim
            )
    ...
    def info(self) -> dict[str, Any]:
        ...
        if hasattr(self.source_data, "dims"):
            ...
            if hasattr(self.source_data, "sizes"):
                source_dims = {
                    dim: self.source_data.sizes[dim] for dim in self.source_data.dims
                }
            else:
                source_dims = {
                    dim: len(self.source_data[dim])
                    if dim in self.source_data.dims
                    else "?"
                    for dim in self.source_data.dims
                }
    ...
    def _create_source_grid_from_data(
        ...
        try:
            ...
            if lat_coords and lon_coords:
                ...
                raise ValueError(
                    "Source data must have at least 2 dimensions for curvilinear regridding"
                )
