...
class InterpolationEngine:
    ...
    def __init__(
        ...
        self.precomputed_weights: dict | None = None
    ...
    def build_structures(
        ...
        if self.method == "nearest":
            ...
            raise ValueError(
                f"Unsupported method: {self.method}. Use 'nearest' or 'linear'"
            )
    ...
    def _build_nearest_neighbour(
        ...
        if radius_of_influence is not None:
        ...
        elif source_points_3d.shape[0] > 1:
            ...
            if unique_points.shape[0] > 1:
                ...
                distances_to_2nd_neighbor, _ = self.source_kdtree.query(
                    source_points_3d, k=2
                )
                # Use twice the mean distance to the 2nd nearest neighbor as a threshold
                if distances_to_2nd_neighbor.ndim == 2:
                    self.distance_threshold = 2 * np.mean(
                        distances_to_2nd_neighbor[:, 1]
                    )
                else:
    ...
    def _precompute_barycentric_weights(
        self, target_points_3d: np.ndarray, source_points_3d: np.ndarray
    ) -> None:
        ...
        for target_idx, target_point in enumerate(points_to_use):
            ...
            if simplex_idx != -1:
                ...
                simplex_vertices = source_points_3d[
                    self.triangles.simplices[simplex_idx]
                ]
                weights = self._compute_barycentric_weights_3d(
                    target_point, simplex_vertices
                )
                ...
                if self.fill_method == "nearest":
                    ...
                    self.precomputed_weights["simplex_indices"][
                        target_idx
                    ] = -2  # Mark as nearest neighbor fallback
                    self.precomputed_weights["valid_points"][target_idx] = True
                    ...
                    if distance < self.distance_threshold:
                        ...
                        self.precomputed_weights["valid_points"][target_idx] = True
                        if not hasattr(self, "_fallback_indices"):
                            self._fallback_indices = np.full(
                                n_targets, -1, dtype=np.int32
                            )
                        self._fallback_indices[target_idx] = nearest_idx

    def _point_in_tetrahedron(
        self, point: np.ndarray, tetra_vertices: np.ndarray
    ) -> bool:
        ...
        return (
            ...
            and np.all(weights <= 1 + 1e-9)
        )

    def _compute_barycentric_weights_3d(
        self, point: np.ndarray, tetra_vertices: np.ndarray
    ) -> np.ndarray | None:
    ...
    def _point_in_triangle_3d(
        self, point: np.ndarray, triangle_vertices: np.ndarray
    ) -> bool:
    ...
    def _compute_barycentric_weights_2d_in_3d(
        self, point: np.ndarray, triangle_vertices: np.ndarray
    ) -> np.ndarray:
    ...
    def interpolate(
        self, source_data: np.ndarray, use_precomputed: bool = True
    ) -> np.ndarray:
    ...
    def _interpolate_nearest(self, source_data: np.ndarray) -> np.ndarray:
        ...
        for i in range(reshaped_data.shape[0]):
            ...
            if (
                self.fill_method == "nan"
                and self.distances is not None
                    ...
                    if valid_mask[j]:
                        ...
                        if not np.isnan(slice_values[nearest_idx]):
                            ...
                            (
                                ...
                                indices_to_neighbors,
                            ) = self.source_kdtree.query(self.target_points_3d[j], k=k)
                ...
                for j in range(
                    len(self.source_indices) if self.source_indices is not None else 0
                ):
                    nearest_idx = (
                        self.source_indices[j] if self.source_indices is not None else 0
                    )
                    if not np.isnan(slice_values[nearest_idx]):
                        result[i, j] = slice_values[nearest_idx]
                    else:
    ...
    def _interpolate_linear(
        ...
        result = np.full(
            (reshaped_data.shape[0], n_targets), np.nan, dtype=source_data.dtype
        )
        ...
        for target_idx in range(n_targets):
            if self.precomputed_weights["valid_points"][target_idx]:
                ...
                if simplex_idx >= 0:  # Valid tetrahedron found
                    ...
                    weights = self.precomputed_weights["barycentric_weights"][
                        target_idx
                    ]
                        ...
                        for slice_idx in range(reshaped_data.shape[0]):
                            ...
                            if np.any(np.isnan(vertex_values)):
                                if self.fill_method == "nearest":
                                    ...
                                    # Make sure indices_to_neighbors is an array
                                    if np.isscalar(indices_to_neighbors):
                                        indices_to_neighbors = np.array(
                                            [indices_to_neighbors]
                                        )
                                    elif not isinstance(
                                        indices_to_neighbors, np.ndarray
                                    ):
                                        indices_to_neighbors = np.asarray(
                                            indices_to_neighbors
                                        )
                                ...
                                result[slice_idx, target_idx] = np.dot(
                                    weights, vertex_values
                                )

                elif (
                    simplex_idx == -2
                ):  # Fallback to nearest neighbor for points outside hull
                    if hasattr(self, "_fallback_indices"):
                        nearest_idx = self._fallback_indices[target_idx]
                        for slice_idx in range(reshaped_data.shape[0]):
                            result[slice_idx, target_idx] = reshaped_data[
                                slice_idx, nearest_idx
                            ]
