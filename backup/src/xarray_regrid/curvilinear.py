...
class CurvilinearInterpolator:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.method_kwargs = {
            k: v for k, v in kwargs.items() if k != "radius_of_influence"
        }
    ...
    def triangles(self):
        ...
        if (
            hasattr(self.interpolation_engine, "triangles")
        ...
        raise AttributeError(
            f"'{self.__class__.__name__}' object has no attribute 'triangles'"
        )
    ...
    def triangle_centroids(self):
        ...
        raise AttributeError(
            f"'{self.__class__.__name__}' object has no attribute 'triangle_centroids'"
        )

    @property
    def triangle_centroid_kdtree(self):
        """Access KDTree of triangle centroids from the interpolation engine."""
        if self.method == "linear" and hasattr(
            self.interpolation_engine, "target_kdtree"
        ):
        ...
        raise AttributeError(
            f"'{self.__class__.__name__}' object has no attribute 'triangle_centroid_kdtree'"
        )
    ...
    def kdtree(self):
        ...
        raise AttributeError(
            f"'{self.__class__.__name__}' object has no attribute 'kdtree'"
        )
    ...
    def target_kdtree(self):
        ...
        raise AttributeError(
            f"'{self.__class__.__name__}' object has no attribute 'target_kdtree'"
        )
    ...
    def convex_hull(self):
        ...
        raise AttributeError(
            f"'{self.__class__.__name__}' object has no attribute 'convex_hull'"
        )
    ...
    def distance_threshold(self):
        ...
        raise AttributeError(
            f"'{self.__class__.__name__}' object has no attribute 'distance_threshold'"
        )
    ...
    def source_indices(self):
        ...
        raise AttributeError(
            f"'{self.__class__.__name__}' object has no attribute 'source_indices'"
        )
    ...
    def _find_triangle_containing_point(self, point_3d, triangle_idx):
        ...
        return self.interpolation_engine._point_in_tetrahedron(
            point_3d, simplex_vertices
        )
    ...
    def _compute_barycentric_weights(self, point_3d, triangle_idx):
        ...
        # Get the triangle vertices
        triangle_vertices = self.source_points_3d[
            self.interpolation_engine.triangles.simplices[triangle_idx]
        ]

        # Use the interpolation engine's method to compute barycentric weights
        weights = self.interpolation_engine._compute_barycentric_weights_3d(
            point_3d, triangle_vertices, self.spherical
        )
        return (
            tuple(weights) if weights is not None else (np.nan, np.nan, np.nan, np.nan)
        )
    ...
    def distances(self):
        ...
        raise AttributeError(
            f"'{self.__class__.__name__}' object has no attribute 'distances'"
        )
    ...
    def _validate_coordinates(self):
        ...
        try:
            ...
            if not lat_coords or not lon_coords:
                raise ValueError(
                    "Source grid must have latitude and longitude coordinates"
                )
        ...
        try:
            ...
            if not lat_coords or not lon_coords:
                raise ValueError(
                    "Target grid must have latitude and longitude coordinates"
                )
        ...
        if source_lat_data.ndim not in [1, 2] or source_lon_data.ndim not in [1, 2]:
            raise ValueError(
                f"Source coordinates must be 1D or 2D. "
        ...
        if target_lat_data.ndim not in [1, 2] or target_lon_data.ndim not in [1, 2]:
            raise ValueError(
                f"Target coordinates must be 1D or 2D. "
    ...
    def _transform_coordinates(self):
        ...
        (
            ...
            self.source_y,
            self.source_z,
        ) = self.coordinate_transformer.transform_coordinates(
            source_lon_flat, source_lat_flat, source_heights
        )

        # Check for finite values before creating 3D points array
        if not (
            np.isfinite(self.source_x).all()
            and np.isfinite(self.source_y).all()
            and np.isfinite(self.source_z).all()
        ):
            # Identify problematic coordinates
            non_finite_mask = ~(
                np.isfinite(self.source_x)
        ...
        self.source_points_3d = np.column_stack(
            [self.source_x, self.source_y, self.source_z]
        )
        ...
        # Check for finite values before creating 3D points array
        if not (
            np.isfinite(self.target_x).all()
            and np.isfinite(self.target_y).all()
            and np.isfinite(self.target_z).all()
        ):
            # Identify problematic coordinates
            non_finite_mask = ~(
                np.isfinite(self.target_x)
        ...
        # Store as 3D points array
        self.target_points_3d = np.column_stack(
    ...
    def _interpolate_dataarray(self, data: xr.DataArray) -> xr.DataArray:
        ...
        if all(dim in data.dims for dim in source_lat_dims):
            ...
            spatial_dims = tuple(
                spatial_dims[:2]
            )  # Take first two matching dimensions that match source shape
        ...
        if non_spatial_dims:
            ...
            expected_size = (
                np.prod(non_spatial_shape) * target_lat_size * target_lon_size
            )
    ...
    def _interpolate_dataset(self, dataset: xr.Dataset) -> xr.Dataset:
        ...
        result_dataset.coords[self.target_lat_name] = self.target_grid[
            self.target_lat_name
        ]
        result_dataset.coords[self.target_lon_name] = self.target_grid[
            self.target_lon_name
        ]
    ...
    def _validate_data_coordinates(self, data: xr.DataArray) -> bool:
        ...
        return len(matching_sizes) >= len(
            expected_sizes
        )  # At least all source sizes should be present
