...
def create_test_grids(nx_source=100, ny_source=50, nx_target=50, ny_target=25):
    ...
    source_ds = xr.Dataset(
        {"temperature": (("y", "x"), np.random.random((ny_source, nx_source)).astype(np.float32), {"units": "K"})},
...
def benchmark_interpolation():
    ...
    interpolator_linear = CurvilinearInterpolator(source_grid=source_ds, target_grid=target_ds, method="linear")
    ...
    interpolator_nearest = CurvilinearInterpolator(source_grid=source_ds, target_grid=target_ds, method="nearest")
